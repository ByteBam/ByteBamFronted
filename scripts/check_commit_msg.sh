...
check_commit_message() {
    ...
    if ! echo "$commit_msg" | grep -qE "^(feat|fix|docs|style|refactor|perf|test|chore|ci|build|revert)\(.*\):.*$"; then
        echo -e "${RED}Error:${NC} Commit message format is incorrect. It should start with one of '${BLUE}feat|fix|docs|style|refactor|perf|test|chore|ci|build|revert(#isuss_id): commit massage${NC}'." >&2
        exit 1
    fi
}
